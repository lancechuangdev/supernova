...
describe("Step 9: Liquidation flow", function () {
  ...
  const WBTC_CRASH_PRICE = 1_000_000_000_000;
  ...
  it("detects liquidation only after collateral value falls below threshold", async function () {
    await createPoolAndMoveToExecution();

    assert.equal(await pool.isLiquidatable(0), false);

    await oracle.setPrice(await borrowToken.getAddress(), WBTC_CRASH_PRICE);

    assert.equal(await pool.isLiquidatable(0), true);
  });

  it("liquidates by selling only enough collateral when repayment is fully covered", async function () {
    await createPoolAndMoveToExecution();
    ...
    assert.equal(await pool.getPoolState(0), 3n);
    assert.equal(data.liquidationAmountLend, requiredRepayment);
    assert.equal(data.liquidationAmountBorrow, ethers.parseEther("1") - collateralToSell);
  });

  it("sells all matched collateral when liquidation cannot fully repay lenders", async function () {
    await createPoolAndMoveToExecution();
    ...
    await pool.connect(bob).claimBorrow(0);
    await oracle.setPrice(await borrowToken.getAddress(), WBTC_CRASH_PRICE);
    await dexRouter.setRate(await borrowToken.getAddress(), await lendToken.getAddress(), ethers.parseEther("10000"));

    await pool.liquidate(0, ethers.parseEther("1"));

    const data = await pool.getPoolData(0);

    assert.equal(await pool.getPoolState(0), 3n);
    assert.equal(data.liquidationAmountLend, ethers.parseEther("10000"));
    assert.equal(data.liquidationAmountBorrow, 0n);
  });

  it("lets SP and JP holders withdraw liquidation proceeds", async function () {
    await createPoolAndMoveToExecution();
  ...
  it("rejects liquidation when healthy, without router, by non-owner, or with too-low max collateral", async function () {
    await createPoolAndMoveToExecution();

    await expectRevert(pool.liquidate(0, ethers.parseEther("1")), "LearningPledgePool: pool not liquidatable");

    await oracle.setPrice(await borrowToken.getAddress(), WBTC_CRASH_PRICE);

    await expectRevert(pool.connect(alice).liquidate(0, ethers.parseEther("1")), "LearningPledgePool: caller is not owner");
    ...
    await expectRevert(
      pool.liquidate(0, collateralToSell - 1n),
    ...
    await expectRevert(
      poolWithoutRouter.liquidate(0, ethers.parseEther("1")),
